...
public class SelectOneMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 1.WHERER절에서 PK 또는 UNIQUE 칼럼과 동등 비교(=)를 수행
			// 2.집계 함수의 결과를 조회
			...
			// rs.next() 메소드 호출 건 = 1행 스캔
			// rs.next() 메소드는 스캔 성공하면 true, 스캔 실패 시 false 반환
			
			
			// 조회 결과 1개인 경우
			// rs.next() 메소드 호출은 1번!
			if(rs.next()) { // 스캔 성공(조회 결과가 있으면)
				
				
			/*	
			rs 객체는 행 전체를 가리키는 포인터(커서) 역할
			rs 객체를 통해서 행을 구성하는 열(칼럼) 정보를 가져올 수 있음
			
			| BOARD | TITLE | CONTENT | HIT | CREATE_DATE |
