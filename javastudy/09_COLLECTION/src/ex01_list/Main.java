...
public class Main {
	...
	public static void m1() {
		...
		// 생성
		// 1.제네릭(Generic)기반
		// 2.생성할 떄 데이터타입을 결정(구체화)
		
		List<String> list = new ArrayList<String>();// < >안에 타입을 적어준다
		...
		list.add("월");  // 인덱스 0
		list.add("화"); //  인덱스 1	
		list.add(0, "일"); // 인덱스 위치 설정. 0일, 1월, 2화가 된다
		...
		// 요소 제거
		// 1.boolean remove(Object obj) : obj제거. 성공하면 true반환
		// 2.Object removeint index9 : index 위치의 요소 제거.제거한 요소 반환
	...
	public static void m3() {
		...
		// 특정 요소를 포함하고 있는가?
		if(list.contains(4)) { // 4를 포함하고 있는가
			System.out.println("4를 포함한다.");
		}else {
			System.out.println("4를 포함하지않는다.");
		}
