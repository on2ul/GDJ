...
public class Main {
	...
	public static void m3() {
		...
		// 파일의 크기
		// 영문은 1바이트, 한글은 2바이트 이상~
		long size=file.length(); // 바이트 단위로 나온다
	...
	public static void m4() {
		...
		File[] list=dir.listFiles(); // 디렉터리 내부의 모든 파일/디렉터리를 File객체로 가져옴
		for(int i=0;i<list.length; i++) {
			System.out.println(list[i].getName()); // list[i] =파일 하나를 뜻한다
	...
	public static void q2() {
		...
		// C:\storage 디렉터리 삭제
		// 디렉터리가 비어 있어야 삭제할 수 있으므로 내부 파일은 먼저 삭제한다
		//
