...
public class XMLReader {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("최상위요소:"+root.getNodeName());
			...
			for(int i=0; i<nodeList.getLength();i++) {
				Node node=nodeList.item(i);	// 줄 바꿈(#text)과 <product>태그로 구성
				if(node.getNodeType()==Node.ELEMENT_NODE) { // 노드가 Elemnet인가?(줄바꿈 #text 제외되고 <product>태그만 남음)
				      ...
				      for(int j=0;j<nodeList2.getLength(); j++) {
				    	  ...
				    	  if(node2.getNodeType()==Node.ELEMENT_NODE) {
				    		  ...
				    		  // System.out.println(node2.getNodeName()+":"+node2.getTextContent()); //TextContent:100,새우깡,1500...
				    	  }
