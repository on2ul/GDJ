...
public class NoticeServiceImpl implements NoticeService {
	...
	public void addNotice(HttpServletRequest request, HttpServletResponse response) {
		...
		int result=mapper.insertNotice(notice); // 0이 넘어가면 실패 1이 넘어가면 성공이다
		...
		try {
			...
			if(result >0) { // if(result ==1)  {
				...
				out.println("location.href='"+request.getContextPath()+"/ntc/list';"); // 자바에서 컨텍스트패스는 리퀘스트로 확인할 수 있다. 로케이션 이동은 리다이렉트 이동이다.
				// out.println("location.href='/notice/ntc/list';"); // 당장은 되지만 미래를 위해서 사용하지 말자
				out.println("</script>");
			} else {
				out.println("<script>");
