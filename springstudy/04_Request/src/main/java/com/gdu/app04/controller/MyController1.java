...
public class MyController1 {
	...
	@GetMapping("detail2")
	public String detail2(@RequestParam(value="id",required=false, defaultValue = "master") String id  // 파라미터 id를 String id에 저장하시오.  required=false 아이디가 꼭 필요하냐 꼭 그렇진 않다
		...
		/*
		 ...
		 3.defaultBalue:파라미터가 없을 때 사용할 값
		 */
	...
	// <foem action="${contextPath}/member/detail4" method="get">
	@GetMapping("detail4")
	public String getdetail4(Member member // 파라미터 id,pw를 setId(), setPw() 메소드를 이용해서 member 객체에 저장해준다
							,Model model) {
		model.addAttribute("member", member);
	...
	// <foem action="${contextPath}/member/detail4" method="post">
	@PostMapping("detail4") // 요청 : URLMApping + 요청 메소드
