      ...
      $('#content').summernote({
         ...
         callbacks:{
        	 ...
        	 onImageUpload:function(files) {
        		 ...
        		 formData.append('file', files[0]); // 파라미터 file, summernote 편집이게 추가된 이미지가 files[0]임 
        		 // 이미지를 HDD에 저장하고 경로를 받아오는 ajax
        		 $.ajax({
        			 type:'post',
        			 url:getContexPath() +'/blog/uploadImage',
        			 data:formData,
        			 contentType:false, // ajax 이미지 첨부용
        			 processData:false, // ajax 이미지 첨부용
        			 dataType: 'json', // HDD에 저장된 이미지 경로를 json으로 받아옴
        			 success: function(resData) {
                         ...
                         mapping=${contextPath}/load/image/aa.jpg인 파일의 
                         location=C:\\upload\aa.jpg 이다.
                         
                         스프링에서 정적 자원 표시하는 방법은 servlte=contet.xml에 있다.
                         이미지(정적 자원)의 mappin과 location을 servlet-context.xml에 작성해야 한다.
