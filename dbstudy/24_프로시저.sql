...
-- 1.PROCEDURE
-- 2.여러개의 쿼리문은 한 번에 실행
--  (이체 : UPDATE문 2개 필요(내꺼 업데이트,니꺼 업데이트)
...
-- 프로시저 PROC1 정의(만들기)
-- 정의하는 부분 호출하는부분 따로 블락 잡아서 돌려야 한다
...
-- 프로시저 PROC2 정의
-- 사원 번호=100인 사원의 FIRST_NAME 서버메시지로 출력하기
...
-- 프로시저 RPOC3 호출
EXECUTE PROC3(100); --EMP_ID로 전달
EXECUTE PROC3(101);
EXECUTE PROC3(500); -- 없는 번호 전달 시 "no data found" 오류 발생




-- 출력 파라미터
-- 1.프로시저의 결과(빤환) 값을 저장하는 변수
-- 2.형식: 변수명 OUT 타입



-- 프로시저 PROC4 정의
-- 사원 번호=100인 사원의 FIRST_NAME을 출력 파라미터 FNAME에 저장하기
...
BEGIN
    PROC4(FNAME); -- PLSQL(프로그래밍) 내부에서 프로시저를 호출 할 땐 EXECUTE 생략
    DBMS_OUTPUT.PUT_LINE(FNAME);
END;
...
-- 프로시저 PROC5 정의
-- 사원 번호가 있으면 FIRST_NAME을 출력 파라미터로 전달, 없으면 'NoName'을 출력 파라미터로 전달
...
CREATE OR REPLACE PROCEDURE PROC6(EMP_ID IN EMPLOYEES.EMPLOYEE_ID%TYPE -- 입력 파라미터 IN  
                                ,FNAME OUT EMPLOYEES.FIRST_NAME%TYPE) -- 출력 파라미터 OUT 
IS
BEGIN
    SELECT FIRST_NAME
      INTO FNAME
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('예외코드'||SQLCODE); -- 예외코드 찍어주는 코드 :SQLCODE
...
-- 구매 프로시저 호출
EXECUTE BUY_PROC(1,1000,10) -- 철수가 진라면을 10개 샀다(고객번호1,제품코드1000,구매수량 10)
