...
--       수행할 PL/SQL
--   END;
--  3.PL/SQL의 데이터(변수,상수)는 서버 메시지로 출력
...
BEGIN
     SNO :='901010-1234567';
     SELECT SUBSTR(SNO,8,1) -- 또는 (SUBSTR(SNO,-7,1)
        INTO GENDER_NUM
        FROM DUAL;
      CASE
...
-- 2) EMPLOYEES 테이블의 모든 사원의 FIRST_NAME,LAST_NAME 조회하기
-- FIRST_NAME과 LAST_NAME을 레코드 변수에 저장하고 해당 값을 조회
-- 레코드 변수는 사원 한 명의 정보만 저장할 수 있으므로 한 명씩저장 후 출력
-- 사원번호는 100~206
...
BEGIN
    FOR N IN 1..10 LOOP
        SELECT MOD(N,2) -- N을 2로 나눈 나머지를
            INTO MODULAR -- MODULAR에 저장한다
            FROM DUAL;
    IF MODULAR =0 THEN 
        RESULT:='짝수';
    ELSE
...
-- 배열의 타입 정의
    TYPE SALARY_TYPE IS TABLE OF EMPLOYEES.SALARY%TYPE INDEX BY BIBINARY_INTEGER;
...
BEGIN
    FOR SAL IN(SELECT SALARY FROM EMPLOYEES)LOOP
...
--     WHERE EMPLOYEE_ID=0; -- 존재하지 않는 사원 조회
        WHERE EMPLOYEE_ID=50; -- 변수에 저장할 데이터가 너무 많음
EXCEPTION
    WHEN NO_DATA_FOUND THEN
     DBMS_OUTPUT.PUT_LINE('조회된 데이터가 없음' || SQLCODE);
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('예외코드' || SQLCODE);
        DBMS_OUTPUT.PUT_LINE('예외코드' || SQLERRM);
END;
