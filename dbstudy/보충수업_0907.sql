...
-- WHERE TO_DATE(O.ORDER_DATE) NOT BETWEEN TO_DATE('14/07/04','YY/MM/DD') AND ON_DATE('14/07/07','YY/MM/DD')
...
-- 구매 이력이 있는 고객만 조회하시오.(주문내역에 있고, 고객에 잇는 데이터 =내부 조인)
-- 고객명  총구매액
-- 박지성  116000
-- 추신수  86000
-- 장미란  62000
...
SELECT C.CUSTOMER_NAME AS 고객명
      ...
      ,COUNT(O.ORDER_ID)AS 구매횟수 -- COUNT에는 고객정보가 포함되면 안됨(고객 테이블은 무조건 포함이라 카운트에 1이 들어감)
...
-- 구매한 고객이 없다면 고객 이름은 NULL로 처리하시오.(책 LECF OUTER JOIN 주문 ...)
-- 고객명  책이름       책가격
-- NULL    골프 바이블  35000
SELECT C.CUSTOMER_NAME AS 고객명
    ,B.BOOK_NAME AS 책이름
