...
-- 메인쿼리와 하위쿼리가 같아야 한다(=메인쿼리는 포지션을 알고싶다 서브쿼리도 포지션으로 써줘야한다)
SELECT EMP_NO,NAME,DEPART,GENDER,POSITION,HIRE_DATE,SALARY
    FROM EMPLOYEE
   WHERE POSITION =(SELECT POSITION -- 단일 행 서브쿼리이므로 연산자 =를 사용,하위쿼리는 메인 쿼리와 동등비교(=)되므로 반드시 POSITION을 반환
                        FROM EMPLOYEE
                        WHERE EMP_NO=1001); --EMP_NO은 PK이므로 단일 행 서브쿼리
...
SELECT EMP_NO,NAME,DEPART,GENDER,POSITION,HIRE_DATE,SALARY
 ...
 SELECT EMP_NO,NAME,GENDER,POSITION,HIRE_DATE,SALARY
        FROM EMPLOYEE
...
-- 6.부서번호가 2인부서에 근무하는 사원들의 직급과 일치하는 직급을 가진 사원 조회하기
SELECT EMP_NO,NAME,GENDER,POSITION,HIRE_DATE,SALARY
    FROM EMPLOYEE 
    WHERE POSITION IN(SELECT POSITION -- 다중 행 서브 쿼리의 동등 비교는 IN연산으로 수행해야 함
                        FROM EMPLOYEE
                        WHERE DEPART=2); --



-- SINGLE-ROW SUBQUERY 오류 발생:단일행 서브쿼리가 하나 이상의 데이터를 반환하고 있음      
--WHERE POSITION=(SELECT LOCATION
--                    FROM DEPARTMENT
--                    WHERE DEPART=2); -- DAPART가 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리 



-- TIP) 단일 행/다중 행 상관 없이 동등 비교는 IN 연산으로 수행 가능
-- POSITION ='과장' 과 POSITION IN ('과장')은 같다
SELECT EMP_NO,NAME,DEPART,GENDER,POSITION,HIRE_DATE,SALARY
    FROM EMPLOYEE
 ...
 SELECT EMP_NO,NAME,DEPART,GENDER,POSITION,HIRE_DATE,SALARY
    FROM EMPLOYEE
        WHERE DEPART IN(SELECT DEPT_NO
                            FROM DEPARTMENT
                            WHERE DEPT_NAME='영업부');-- DAPT_NAME이 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리 
...
WHERE DEPT_NO IN(SELECT DEPART
                    FROM EMPLOYEE
                   WHERE POSITION = '과장');-- PSITION이 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리                 
...
-- WHERE SALARY>ANY(200000,5000000)
-- SALARY가 2000000보다 크거나, 5000000보다 크면 됨(OR개념)
-- 따라서 최소급여 2000000보다 크면 만족하는 상황임
...
--                    FROM EMPLOYEE
--                   WHER DEPART =1); -- DAPART가 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리 
            



-- 10.부서번호가 1인 부서에 근무하는 사원들의 급여보다 더 많은 급여를 받는 사원 조회하기
-- 모든 급여와 비교해서 많이 받으면 조회하기
 SELECT EMP_NO,NAME,DEPART,GENDER,POSITION,HIRE_DATE,SALARY
            FROM EMPLOYEE 
...
-- WHERE SALARY>ALL(200000,5000000)
-- SALARY가 2000000보다 크고, 5000000보다 크다(AND 개념)


-- SALARY>ANY(2000000,5000000) -> SALARY>(SELECT MIN(2000000,5000000)...)
-- SALARY>ALL(2000000,5000000) -> SALARY>(SELECT MAX(2000000,5000000)...)
...
SELECT ROWID,EMP_NO,NAME
 FROM EMPLOYEE
WHERE ROWID='AAAE/kAABAAALDBAAD';
...
-- ROWNUM 사용 방법
-- 1.ROWNUM은 1을 포함하는 범위는 조건으로 사용 할 수 있음
...
SELECT EMP_NO,NAME
    FROM EMPLOYEE
  WHERE ROWNUM=2; -- 1이 포함되어 있지 않아서 조회가 불가능하다
...
-- 2: WHERE(ROWNUM의 별명 사용하기)
 ...
 SELECT ROWNUM AS ROW_NUM,EMP_NO,NAME 
    FROM EMPLOYEE
   WHERE ROW_NUM =2; -- 오류 발생 실행 순서에 의해 별명인 ROW_NUM을 사용할 수 없다 
   
   
-- 인라인뷰를 이용해 실행 순서를 바꿔준다  
...
-- 정렬한 결과에 번호를 붙이고 싶다면 바깥쪽 SELECT문에 ROWNUM을 쓴다
...
-- 1)ROWNUM 칼럼
-- 인라인 뷰 A: 연봉으로 정렬한 테이블
-- 인라인 뷰 B: 정렬이 끝난 테이블에 행 번호를 추가한 테이블
...
CREATE TABLE DEPARTMENT2
    AS(SELECT DEPT_NO,DEPT_NAME,LOCATION
        FROM DEPARTMENT
         WHERE 1=2); -- 절대 만족하지 않는 조건을 넣어준다
         
-- 행을 제외하고 복사되었는지 확인
SELECT DEPT_NO,DEPT_NAME,LOCATION FROM DEPARTMENT2; -- 데이터가 하나도 없다
...
-- 3.DEPARTMENT 테이블이 모든 행(ROW)을 DEPARTMENT2테이블에 삽입하기
...
-- 사원번호 1004의 정보를 사원번호 1001로 복사하기
UPDATE EMPLOYEE2
...
SELECT EMP_NO,NAME,GENDER FROM EMPLOYEE2; -- 변경됐는지 확인


-- 사원번호 1003의 정보를 사원번호 1001로 복사하기
UPDATE EMPLOYEE2
