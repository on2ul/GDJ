...
-- JOIN
-- 1.2개 이상의 테이블을 조회하는 방법
-- 2.조회할 테이블들은 관계를 줄 수 있어야 함
-- 3.종류
...
-- 3.조인할 때 드라이브 테이블을 드리븐 테이블보다 먼저 작성해야 한다
...
-- 모두 포함 시킬 테이블이 기준
-- 모두 포함시킬 사원 테이블을 OUTER JOIN의 왼쪽/오른쪽에 두느냐에 따라
-- 왼쪽 외부 조인/오른쪽 외부 조인으로 구분함


--DRIVER/DRIVEN 테이블이 잘못 지정된 조인
SELECT E.EMP_NO,E.NAME,D.DEPT_NAME
  FROM EMPLOYEE E LEFT OUTER JOIN DEPARTMENT D -- 왼쪽 EMPLOYEE 테이블은 모두 조회
    ON E.DEPART=D.DEPT_NO;

--DRIVER/DRIVEN 테이블이 잘 지정된 조인
-- LEFT/RIGHT의 결과 값은 같지만 RIGHT의 성능이 더 빠르다
SELECT E.EMP_NO,E.NAME,D.DEPT_NAME
    FROM DEPARTMENT D RIGHT OUTER JOIN EMPLOYEE E -- 오른쪽 EMPLOYTEE 테이블은 모두 조회
    ON D.DEPT_NO =E.DEPART;
