-- 그룹(GROUP BY)
-- 1.GORUP BY절에서 지정한 칼럼의 데이터는 한 번만 조회가 됨
...
--2.그룹화 실패
-- EMPLOUYEE_ID를 조회하려면 GROUP BY절에 EMPLOYEE_ID가 있어야 함
SELECT EMPLOYEE_ID
    FROM EMPLOYEES
    GROUP BY DEPARTMENR_ID;
 
 
 
-- 3.그룹화 함수 사용
-- 집계함수(그룹함수)는 GROUP BY절에 해당 칼럼이 없어도 SELECT절에서 조회
SELECT
      DEPARTMENT_ID
