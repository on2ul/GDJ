...
-- 조인으로 풀어야함
-- DEPARTMENTS 테이블 소속 ->DEPARTMENT_NAME
-- EMPLOYEES 테이블 소속 -> EMPLOYEE_ID, FIRST_NAME, DEPARTMENT_ID
-- DEPARTMENT_ID 는 양쪽 다 소속되어 있기 때문에 어디있는걸 꺼내써도 상관 없다
...
-- 타입이 섞여 있는 경우 오름차순 정렬
-- 문자->숫자->날짜->NULL


-- 2. DEPARTMENT_ID가 20인 사원중에서 전체 평균 연봉(SALARY) 이상을 받는 사원의 EMPLOYEE_ID, FIRST_NAME, DEPARTMENT_ID, SALARY를 조회하시오.

-- EMPLOYEES 테이블 소속->DEPARTMENT_ID
-- EMPLOYEES 테이블 소속->연봉(SALARY)
-- EMPLOYEES 테이블 소속->EMPLOYEE_ID, FIRST_NAME, DEPARTMENT_ID, SALARY
-- 전부 한 테이블에 있는것이니 조인으로 푸는거 x
...
-- 메인쿼리:SELECT 칼럼FROM 테이블 WHERE (JOB_ID,DEPARTMENT_ID) = (서브쿼리)  : 단일 행 -> 사원번호는 PK이다
-- 메인쿼리:SELECT 칼럼FROM 테이블 WHERE (JOB_ID,DEPARTMENT_ID) IN (서브쿼리) : 다중 행
...
-- MANAGER가 아닌 사원: MANAGER_ID가 MANAGER_ID에 없는 사원
...
-- 가장 많은 사원이 근무 중인 부서 : 부서별 사원수를 구해야 알 수 있다->GROUP BY
-- 조건: 근무중인 사원수 = 최대 사원수
-- HAVING절?WHERE절?
-- 집계함수를 이용한 조건은 100% HAVING절만 가능하다
...
-- 1)인라인뷰 A : 최대 연봉이 맨 위에 있는 테이블
-- 2)인라인뷰 B : 연봉순으로 정렬된 테이블에 행 번호(RN)를 부착시켜 둔 테이블
-- 3)최종결과 : 인라인뷰 B에서 행 번호(RN)가 1인 행 조회
