...
--integrity constraint (SCOTT.FK_EMPLOYEE_DEPARTMENT) violated - parent key not found 오류 발생
/*
...
-- 2)부서 번호가 3인 부서명을 '전략부',지역을 '부산'으로 수정
UPDATE DEPARTMENT
    SET DEPT_NAME='전략부'
       ,LOCATION='부산'
 WHERE DEPT_NO=3; 
 ...
 -- 3)부서 번호가 3인 부서의 부서번호를 6으로 수정
 -- 데이터 무결성을 위해 사원을 먼저 변경해야 한다
...
-- 4)부서 번호가 1인 사원들의 월급을 100000인상

UPDATE EMPLOYEE
    SET SALARY=SALARY+100000
WHERE DEPART=1;

-- 5)직급이 '과장'인 직원들의 월급을 10% 인상
-- SALARY  * 1+SALARY*0.1
-- SALARY  * (1+0.1)
-- =SALARY * 1.1
...
-- 부서번호가 1인행(ROW)의 소속부서가 NULL 값으로 함께 변경
DELETE
  FROM DEPARTMENT
 WHERE DEPT_NO=1;
