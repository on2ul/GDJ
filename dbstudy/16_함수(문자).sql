...
SELECT
   ...
   ,LENGTH('안녕'),LENGTHB('안녕') -- 한글 하나에 3BYTE
  FROM
      DUAL;
...
--   (1) CONCAT 함수의 인수(ARUGUMENTS)는 2개만 지원
--   (2) A와 B와 C의 연결
--      ①CONCAT(A,B,C)         :불가능함
...
-- 6.문자열 채우기(PADDING)
--  1)LPAD(칼럼,전체 폭,채울 문자)
--  2)RPAD(칼럼,전체 폭,채울 문자)
SELECT
      LPAD(DEPARTMENT_ID,3,0) --왼쪽에 0을 채워 3자리로 조회
...
SELECT
     RPAD(SUBSTR(EMAIL,1,2),5,'*') -- 오른쪽에 *을 채워서 5자리로 조회하겠다.
    FROM
        EMPLOYEES;
