...
-- 3.DML 직전에 수행되는 BEFORE 트리거, 직후에 수행되는 AFTER트리거가 있음
-- 4.기본적으로 작업이 수행되는 행(ROW)단위로 트리거가 적용됨
-- 5.형식
--  CREATE [OR REPLACE] TRIGGER 트리거_이름
--  [AFTER | BEFORE]
--  [INERST OR UPDATE OR DELETE]
--  [ON 테이블_이름]
--  [FOR EACH ROW]
--  BEGIN
--  트리거 작업
...
AFTER -- AFTER,BEFORE 
    UPDATE -- INERST OR UPDATE OR DELETE 
...
-- 트리거 TRIG3 동작 확인
UPDATE EMPLOYEE SET NAME = '김창민' WHERE EMP_NO = 1001;
