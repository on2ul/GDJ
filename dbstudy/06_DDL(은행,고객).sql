...
CREATE TABLE CUSTOMER(
    ...
    NANE VARCHAR2(30 BYTE) NOT NULL, --필수
...
--4.칼럼명 변경:ALTER TABLE 테이블명 RENAME COLUMN OLD이름 TO NEW이름
--5.테이블 이름:ALTER TABLE 테이블명 RENAME TO NEW이름


-- 1. BANK 테이블에 BANK_PHONE 칼럼을 추가하시오.
ALTER TABLE BANK
    ADD BANK_PHONE VARCHAR2(20 BYTE) NOT NULL;

-- 2. CUSTOMER 테이블에 GRADE 칼럼을 추가하시오. ('VIP', 'GOLD', 'SILVER' 중 하나의 값만 가진다.)
ALTER TABLE CUSTOMER
    ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE IN('VIP','GOLD','SILVER')); --텍스트는 작은 따옴표로 묶어준다

-- 3. BANK 테이블의 BANK_NAME 칼럼을 VARCHAR2(15 BYTE)로 수정하시오.
-- NULL 유무를 지정해주지 않으면 본래 갖고있던 값을 가져다가 사용한다
ALTER TABLE BANK
