...
public class BoardTest {
	 ...
	 */
		...
		public void 상세조회테스트() {
			...
			// 6번 게시글의 제목이 '하이룽 ㅎㅇㅎㅇ'이면 테스트 성공, 아니면 실패
			/*
			Board board= BoardDao.getInstance().selectBoardByNo(6);
			assertEquals("하이룽ㅎㅇㅎㅇ", board.getTitle());
			*/
			
			// 6번 게시글을 가진 게시글이 있으면 테스트 성공, 아니면 실패
			Board board=BoardDao.getInstance().selectBoardByNo(6);
		...
		public void 게시글수정테스트() {
			...
			board.setBoardNo(8); // 게시글 삽입테스트 결과로 삽입된 게시글 번호
		...
		public void 게시글삭제테스트() {
			...
			int result=BoardDao.getInstance().deleteBoard(8);
			assertEquals(1, result);
		}
