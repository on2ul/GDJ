...
public class MyServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//	  2) HttpServaletRequest request 객체가 처리(Tomcat이 있어야 사용 가능)
		
		
		// 1) 요청이 포함된 한글 처리(문자셋:UTF-8)
		...
		// null인지 아닌지 걸러내기 ( 500 오류 뜨기 때문에 null을 처리를 해줘야 함)
		int age =0;
		if(strAge!=null) {
		...
		// 2. 응답
		//	1) 서버 -> 클라이언트로 보내는 응답
		//	2) HeepServaletReponse response 객체가 처리(Tomcat이 있어야 사용 가능)
